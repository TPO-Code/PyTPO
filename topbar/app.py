...
def main(argv: list[str] | None = None) -> int:
    ...
    if not instance_guard.acquire():
        logger.info("startup timing: instance-guard blocked after %.1f ms", (time.perf_counter() - acquire_started) * 1000.0)
        logger.info("another topbar instance is already running; exiting")
        return 0
    logger.info("startup timing: instance-guard acquired in %.1f ms", (time.perf_counter() - acquire_started) * 1000.0)
    ...
    logger.info("startup timing: QApplication created in %.1f ms", (time.perf_counter() - app_started) * 1000.0)
    ...
    bar = TopBar()
    logger.info("startup timing: TopBar constructed in %.1f ms", (time.perf_counter() - bar_started) * 1000.0)
    bar.show()
    logger.info("startup timing: pre-event-loop startup completed in %.1f ms", (time.perf_counter() - startup_started) * 1000.0)
    return app.exec()
