...
PyTPO supports two theme source formats:
...
- `src/themes/Midnight Forge.qsst`
- `src/themes/Deep Night.qsst`
- `src/themes/_Midnight Forge.qss` (legacy backup/reference)
- `src/themes/_Deep Night.qss` (legacy backup/reference)
...
PyTPO picks `.qsst` first (current extension priority is `(".qsst", ".qss")`).
...
- PyTPO re-applies that active theme automatically
