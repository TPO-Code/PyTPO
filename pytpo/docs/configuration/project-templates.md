# Project Templates (`.pytpo/templates/*.json`)

PyTPO New Project templates are loaded from IDE-scope JSON files under:
...
- All `.json` files in `.pytpo/templates/` are loaded.
...
- If no valid templates are loaded, PyTPO falls back to built-in defaults.
...
Create `.pytpo/templates/tdoc-pack.json`:
...
Restart PyTPO (or reopen New Project dialog) and select `TDOC Notes Pack`.
