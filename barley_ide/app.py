...
from barley_ide.app_launch_monitor import extract_monitor_launch, run_monitored_process
...
from barley_ide.storage_paths import migrate_legacy_ide_storage
from barley_ide.ui.python_ide import PythonIDE, request_project_activation
...
def main(argv: list[str] | None = None) -> int:
    ...
    if hasattr(app, "setDesktopFileName"):
        app.setDesktopFileName("barley-ide")
    ...
    if lingering:
        ...
        try:
            print(
                f"[Barley] Forcing process exit due to lingering worker threads: {preview}",
                file=sys.stderr,
                flush=True,
            )
        except Exception:
