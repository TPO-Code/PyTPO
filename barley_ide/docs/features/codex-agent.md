...
Barley includes a docked chat interface for `codex` CLI.  
...
2. an open project directory in Barley
...
- Barley only adds `--skip-git-repo-check` automatically when the project is not under source control.
...
Recommended screenshot: `barley_ide/docs/assets/screenshots/16-codex-agent-dock.png`
