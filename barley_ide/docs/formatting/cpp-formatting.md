...
Barley formats C/C++ via `clang-format -style=file`.

## Style file discovery

Barley searches upward for `.clang-format` from the file directory:
...
If missing, Barley opens `No .clang-format found` with:
...
Recommended screenshot: `barley_ide/docs/assets/screenshots/13-format-missing-clang-format-dialog.png`
