# Barley Documentation

User documentation for running, configuring, and troubleshooting Barley.

![Documentation Map](assets/images/docs-map.svg)

## Start Here

- [Getting Started](getting-started.md)
- [Daily Development Workflow](workflows/daily-development.md)
- [Troubleshooting](troubleshooting.md)

## Concepts

- [Concepts Index](concepts/README.md)
- [Projects and `.tide`](concepts/projects-and-tide.md)
- [Interpreter Selection](concepts/interpreter-selection.md)
- [Indexing and Search](concepts/indexing-and-search.md)
- [Run and Terminal Model](concepts/run-and-terminal-model.md)

## Features

- [Features Index](features/README.md)
- [Editor Basics](features/editor.md)
- [Panels](features/panels.md)
- [Workspace Slots](features/workspace-slots.md)
- [Linting and Problems](features/linting.md)
- [Find and Replace in Files](features/find-replace-in-files.md)
- [Git Integration](features/git.md)
- [AI Assist](features/ai-assist.md)
- [Codex Agent Dock](features/codex-agent.md)

## Configuration

- [Configuration Index](configuration/README.md)
- [Project Templates (`.barley_ide/templates/*.json`)](configuration/project-templates.md)
- [Themes (`.qss` and `.qsst`)](configuration/themes.md)
- [IDE Settings](configuration/ide-settings.md)
- [Run Configurations](configuration/run-configurations.md)
- [Project Configuration (`.tide/project.json`)](configuration/project-json.md)
- [Project JSON Reference](configuration/project-json-reference.md)

## Language Support

- [Language Support Index](languages/README.md)
- [C/C++ Support (clangd)](languages/cpp-support.md)
- [Rust Support (rust-analyzer + Cargo)](languages/rust-support.md)
- [TDOC Support (`.tdoc` + `.tdocproject`)](languages/tdoc-support.md)

## Formatting

- [Formatting Index](formatting/README.md)
- [Python Formatting (Ruff)](formatting/python-formatting.md)
- [C/C++ Formatting (clang-format)](formatting/cpp-formatting.md)
- [Rust Formatting (cargo fmt / rustfmt)](formatting/rust-formatting.md)

## Screenshots

- [Screenshot Shot List](screenshot-shotlist.txt)
- Place screenshots under `barley_ide/docs/assets/screenshots/`
