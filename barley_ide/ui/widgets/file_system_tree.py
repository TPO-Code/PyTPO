...
from barley_ide.ui.icons.file_icon_provider import FileIconProvider
...
class FileSystemTreeModel(QAbstractItemModel):
    ...
    def __init__(
        ...
        self._git_visible_folder_states: Dict[str, str] = {}
    ...
    def refresh_tree(self, include_excluded: bool = False):
        ...
        self._rebuild_git_visible_folder_states()
    ...
    def set_git_tinting(self, *, enabled: bool, colors: Dict[str, str]):
        ...
        for key in ("clean", "dirty", "untracked"):
            ...
            if val:
                merged[key] = val
        if new_enabled == self._git_tint_enabled and merged == self._git_colors:
            return
        self._git_tint_enabled = new_enabled
        self._git_colors = merged
        self._emit_state_update_all()

    def set_git_status_maps(self, *, file_states: Dict[str, str], folder_states: Dict[str, str]):
        previous_file_states = dict(self._git_file_states)
        previous_visible_folder_states = dict(self._git_visible_folder_states)
        ...
        self._rebuild_git_visible_folder_states()
        changed_paths = self._changed_git_state_paths(previous_file_states, self._git_file_states)
        changed_paths.update(self._changed_git_state_paths(previous_visible_folder_states, self._git_visible_folder_states))
        self._emit_state_update_for_paths(changed_paths)
    ...
    def refresh_subtree(self, path: str, include_excluded: Optional[bool] = None):
        ...
        if node == self._root_node or target == self._root_path:
            project_root_node = self._path_map.get(self._root_path)
            if project_root_node is None or project_root_node.parent != self._root_node:
                self.refresh_tree(include_excluded=self._include_excluded)
                return
            self._reload_node_children(project_root_node)
            return
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> Any:
        ...
        if role == Qt.ItemDataRole.ForegroundRole:
            if not self._git_tint_enabled:
                return None
            state = self._git_state_for_node(node.path, node.is_dir)
            if not state:
                return None
    ...
    def _reload_node_children(self, node: _FsNode):
        ...
        old_keys = [(child.path, bool(child.is_dir)) for child in node.children]
        new_keys = [(child.path, bool(child.is_dir)) for child in children]
        ...
        for tag, i1, i2, j1, j2 in reversed(opcodes):
            if tag in ("delete", "replace") and i1 < i2:
                self.beginRemoveRows(parent_index, i1, i2 - 1)
                for child in list(node.children[i1:i2]):
                    self._unregister_node_recursive(child)
                del node.children[i1:i2]
                self.endRemoveRows()
            if tag in ("insert", "replace") and j1 < j2:
                new_segment = children[j1:j2]
                self.beginInsertRows(parent_index, i1, i1 + len(new_segment) - 1)
                for child in new_segment:
                    child.parent = node
                    self._path_map[child.path] = child
                node.children[i1:i1] = new_segment
                self.endInsertRows()
    ...
    def _git_state_for_node(self, path: str, is_dir: bool) -> str:
        if is_dir:
            return str(self._git_visible_folder_states.get(path, "") or "").strip().lower()
        return str(self._git_file_states.get(path, "") or "").strip().lower()

    def _rebuild_git_visible_folder_states(self) -> None:
        folder_states: Dict[str, str] = {}
        for file_path, state in self._git_file_states.items():
            if not self._is_within_root(file_path):
                continue
            if not self._path_contributes_to_visible_git_state(file_path, is_dir=False):
                continue
            folder = self._canonical(os.path.dirname(file_path))
            while self._is_within_root(folder):
                if not self._path_contributes_to_visible_git_state(folder, is_dir=True):
                    break
                current = folder_states.get(folder)
                folder_states[folder] = self._merge_git_state_priority(current, state)
                ...
                if parent == folder:
                    break
                folder = parent
        self._git_visible_folder_states = folder_states

    def _path_contributes_to_visible_git_state(self, path: str, *, is_dir: bool) -> bool:
        cpath = self._canonical(path)
        if cpath == self._root_path or self._include_excluded:
            return True
        if self._exclude_path_predicate is not None:
            try:
                if self._exclude_path_predicate(cpath, is_dir):
                    return False
            except Exception:
                pass
        if is_dir and self._exclude_dir_predicate is not None:
            try:
                if self._exclude_dir_predicate(cpath):
                    return False
            except Exception:
                pass
        return True
    ...
    @staticmethod
    def _merge_git_state_priority(current: str | None, incoming: str) -> str:
    ...
    @staticmethod
    def _changed_git_state_paths(previous: Dict[str, str], current: Dict[str, str]) -> set[str]:
        changed: set[str] = set()
        for path in set(previous.keys()) | set(current.keys()):
            if str(previous.get(path, "") or "") != str(current.get(path, "") or ""):
                changed.add(path)
        return changed
...
class FileSystemTreeWidget(QTreeView):
    ...
    def refresh_subtree(self, path: str, include_excluded: Optional[bool] = None):
        ...
        vpos = int(vbar.value()) if vbar is not None else 0
        hpos = int(hbar.value()) if hbar is not None else 0
        target = self._model._canonical(path)
        expanded_paths = self._collect_expanded_paths_under(target)
        selected_paths = [
            selected
            for selected in self.selected_paths()
            if selected == target or selected.startswith(target + os.sep)
        ]
        current = self.path_from_index(self.currentIndex())
        current_in_target = (
            isinstance(current, str)
            and (current == target or current.startswith(target + os.sep))
        )
        self._model.refresh_subtree(target, include_excluded=include_excluded)
        self._restore_expanded_paths(expanded_paths)
        if selected_paths:
            self._restore_selected_paths(selected_paths, current_path=current if current_in_target else None)
        if vbar is not None:
            vbar.setValue(max(vbar.minimum(), min(vbar.maximum(), vpos)))
        if hbar is not None:
            hbar.setValue(max(hbar.minimum(), min(hbar.maximum(), hpos)))
    ...
    def _collect_expanded_paths_under(self, root_path: str) -> set[str]:
        root_index = self._model.index_from_path(root_path)
        if not root_index.isValid():
            return set()

        expanded: set[str] = set()

        def walk(index: QModelIndex) -> None:
            if not index.isValid():
                return
            path = self._model.path_from_index(index)
            if isinstance(path, str) and self.isExpanded(index):
                expanded.add(self._model._canonical(path))
            node = cast(_FsNode, index.internalPointer())
