...
from barley_ide.ai.context_assembler import ContextAssembler
from barley_ide.ai.inline_controller import InlineSuggestionController
from barley_ide.ai.openai_compatible_client import OpenAICompatibleClient
from barley_ide.ai.settings_schema import normalize_ai_settings
from barley_ide.core.keybindings import get_action_sequence, normalize_keybindings
from barley_ide.formatting import CodeFormattingRegistry, FormatRequest
from barley_ide.formatting.providers import (
...
from barley_ide.git.github_auth import GitHubAuthStore
from barley_ide.git.github_share_service import GitHubShareService
from barley_ide.git.git_service import GitService
from barley_ide.git.workspace_repository_index import WorkspaceRepositoryIndex
from barley_ide.instance_coordinator import ProjectInstanceServer, request_project_activation
from barley_ide.lang_cpp import CppLanguagePack
from barley_ide.lang_cpp.clangd_repair import missing_std_header_from_diagnostic, repair_clangd_includes
from barley_ide.lang_rust import RustLanguagePack
from barley_ide.services.commit_md import (
...
from barley_ide.services.file_open_classifier import FileOpenKind, classify_file_for_open
from barley_ide.services.project_policy_service import ProjectPolicyService
from barley_ide.storage_paths import ide_config_dir, ide_no_project_workspace_dir, migrate_legacy_ide_storage
from TPOPyside.shared_assets import shared_theme_dir

from barley_ide.ui.console_run_manager import ConsoleRunManager
from barley_ide.ui.completion_manager import CompletionManager
from barley_ide.ui.controllers import (
    ActionRegistry,
...
from barley_ide.ui.editor_change_highlight_service import EditorChangeHighlightService
from barley_ide.ui.icons.asset_icons import (
    BUG_ICON_NAME,
    PLAY_ICON_NAME,
    SETTINGS_ICON_NAME,
    STOP_ICON_NAME,
...
from barley_ide.ui.dialogs.find_in_files_dialog import FindInFilesDialog
from barley_ide.ui.dialogs.file_dialog_bridge import get_save_file_name
from barley_ide.ui.dialogs.terminal_commands_dialog import TerminalCommandsDialog
from barley_ide.ui.dialogs.workspace_slot_name_dialog import WorkspaceSlotNameDialog
from barley_ide.ui.settings_dialog import SettingsDialog as ScopedSettingsDialog, create_default_settings_schema
from barley_ide.ui.syntax_highlighting_runtime import configure_syntax_highlighting_runtime

from barley_ide.ui.editor_workspace import EditorTabs, EditorWidget, EditorWorkspace
from barley_ide.ui.lint_manager import LintManager
from barley_ide.ui.spellcheck_manager import SpellcheckManager
from barley_ide.ui.debugger import DebuggerDockWidget
from barley_ide.ui.widgets.code_editor import CodeEditor
from barley_ide.ui.widgets.codex_agent_dock import CodexAgentDockWidget
...
from barley_ide.ui.widgets.markdown_editor_tab import MarkdownEditorTab
from barley_ide.ui.widgets.sound_player_editor import SoundPlayerEditorWidget
from barley_ide.ui.widgets.svg_editor_tab import SvgEditorTab
from TPOPyside.widgets.terminal_widget import TerminalWidget
from barley_ide.ui.widgets.problems_panel import ProblemsPanel
from barley_ide.ui.widgets.symbol_outline_panel import SymbolOutlinePanel
from barley_ide.ui.widgets.tdoc_document_widget import TDocDocumentWidget
from barley_ide.ui.widgets.unified_diff_editor import UnifiedDiffEditor
from barley_ide.ui.widgets.usages_panel import UsagesPanel
from barley_ide.ui.widgets.welcome_screen import WelcomeScreenWidget
...
class PythonIDE(Window):
    ...
    def __init__(self):
        ...
        self._project_fs_dir_signatures: dict[str, tuple[tuple[str, bool], ...]] = {}
        ...
        self._project_fs_refresh_timer.setInterval(1200)
        ...
        self._project_fs_watch_sync_timer.setInterval(1500)
    ...
    def refresh_workspace_repository_index(self, *, update_tree: bool = True) -> WorkspaceRepositoryIndex:
        current_index = getattr(self, "workspace_repository_index", None)
        current_key = (
            current_index.structural_key()
            if isinstance(current_index, WorkspaceRepositoryIndex)
            else None
        )
        refreshed_index = WorkspaceRepositoryIndex.discover(
            self.project_root,
            canonicalize=self._canonical_path,
        )
        refreshed_key = refreshed_index.structural_key()
        changed = refreshed_key != current_key
        self.workspace_repository_index = refreshed_index
    ...
    def _on_project_fs_directory_changed(self, path: str) -> None:
        ...
        if not cpath:
            return
        previous = self._project_fs_dir_signatures.get(cpath)
        current = self._tree_directory_signature(cpath)
        if current is not None:
            self._project_fs_dir_signatures[cpath] = current
        if previous is not None and current is not None and previous == current:
            return
        if not self._tree_directory_needs_refresh(cpath):
            self.schedule_git_status_refresh(delay_ms=120)
            return
        self._queue_project_fs_refresh(cpath)
        self._schedule_project_fs_watch_sync()
    ...
    def _sync_project_fs_watches(self) -> None:
        ...
        if remove_paths:
            ...
            for path in remove_paths:
                self._project_fs_dir_signatures.pop(self._canonical_path(path), None)
        ...
        known = set(self._project_fs_watched_dirs)
        stale = [path for path in self._project_fs_dir_signatures.keys() if path not in known]
        for path in stale:
            self._project_fs_dir_signatures.pop(path, None)
        for path in known:
            signature = self._tree_directory_signature(path)
            if signature is not None:
                self._project_fs_dir_signatures[path] = signature
    ...
    def _tree_directory_signature(self, path: str) -> tuple[tuple[str, bool], ...] | None:
        cpath = self._canonical_existing_watch_dir(path)
        if not cpath:
            return None
            ...
            with os.scandir(cpath) as it:
                for entry in it:
                    try:
                        ...
                        continue
                    abs_path = self._canonical_path(entry.path)
                    if self._is_tree_path_excluded(abs_path, is_dir):
                        continue
                    entries.append((entry.name, is_dir))
        except Exception:
            return None
        entries.sort(key=lambda item: (not item[1], item[0].lower(), item[0]))
        return tuple(entries)

    def _tree_directory_needs_refresh(self, path: str) -> bool:
        cpath = self._canonical_existing_watch_dir(path)
        if not cpath:
            return False
        ...
        if not isinstance(tree, FileSystemTreeWidget):
            return False
        try:
            index = tree.model().index_from_path(cpath)
        except Exception:
            return False
        try:
        ...
        if isinstance(selected, str) and selected:
            base = selected if os.path.isdir(selected) else os.path.dirname(selected)
            if self._canonical_existing_watch_dir(base) == cpath:
                return True
        return False
    ...
    def _refresh_git_branch_status_label(self, *, branch: str | None = None) -> None:
        ...
        label.setText(f"Git: {self._repo_display_label(repo_root)}: {shown_branch}")
    ...
    def _load_toolbar_icon(self, icon_key: str, *, foreground: str | None = None) -> QIcon:
        ...
        if key and key not in self._toolbar_missing_icon_keys:
            ...
            print(f"[Barley] Missing toolbar icon asset '{asset_name}'. Falling back to text.")
            self._append_debug_output_lines(
                [
    ...
    def _documentation_root_candidates(self) -> list[Path]:
        ...
        package_docs_root = Path(__file__).resolve().parents[1] / "docs"
        for candidate in (
    ...
    def open_check_for_updates_dialog(self) -> None:
        from barley_ide.services.update_service import UpdateService
        from barley_ide.ui.dialogs.check_for_updates_dialog import CheckForUpdatesDialog
    ...
    def _about_markdown_payload(self, *, version: str) -> tuple[str, QUrl]:
        about_path = Path(__file__).resolve().parents[1] / "docs" / "about.md"
        text = ""
    ...
    def show_about_dialog(self) -> None:
        from barley_ide.ui.dialogs.about_dialog import AboutDialog
