...
- debugger code lives in `barley_ide/ui/debugger/`
...
- module entry resolution now understands common `barley_ide/` package layouts, and the live debugpy suite includes a package/module launch case for that project shape
...
Continue in `barley_ide/ui/debugger/` and `barley_ide/ui/controllers/execution_controller.py`, starting with deeper backend hardening and broader launch coverage.
...
Debugger integration belongs in `barley_ide/`, not `TPOPyside`.

## Current debugger shape

- `barley_ide/ui/debugger/dock_widget.py` manages session tabs, shared controls, and persisted watch ownership
- `barley_ide/ui/debugger/session_widget.py` owns one debugger backend/controller pair per tab
- `barley_ide/ui/debugger/python_backend.py` is now the backend-selection seam, `barley_ide/ui/debugger/debugpy_backend.py` owns the active DAP-backed Python runtime path, and `barley_ide/ui/debugger/lldb_dap_backend.py` owns the Rust native-debug path
- `barley_ide/ui/debugger/breakpoint_store.py` owns persisted breakpoints and watches
